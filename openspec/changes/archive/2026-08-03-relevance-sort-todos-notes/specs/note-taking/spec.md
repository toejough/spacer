...
The system SHALL allow the user to abandon a note (archive it). An abandoned note remains visible in the note list, sorted last per the state tier of the note list ordering, and can be reopened.
...
The system SHALL order the note list by relevance: state first (active before abandoned), then review urgency (soonest due review first, with items that have review disabled or no scheduled review ranked after any item with a due date), then recency (most recently updated first) as the final tiebreaker.
...
- **WHEN** a note has reviews disabled or no scheduled review, and another note in the same state tier has a due date
- **THEN** the item with a due date appears first

#### Scenario: Multi-cloze items use their earliest due date
- **WHEN** a note has multiple cloze deletions with different `next_review` dates
...
- **WHEN** two notes have the same review-urgency ranking
