...
func TestFetchFilesHappyPath(t *testing.T) {
	...
	rapid.Check(t, func(test *rapid.T) {
        ...
        protest.MustEqual(test, []any{}, call.MustPullArgs(test))
...
func fetchFilesToMutate(deps *fetchFilesDeps) []filepath {
	panic("unimplemented")
}
...
func newFetchFilesMock(test tester) (*protest.FIFO[protest.AnyCall], *fetchFilesDeps) {
	...
	return calls, &fetchFilesDeps{
		fetchPaths: func() []filepath {
			return protest.ProxyCall(test, calls, "fetchPaths").([]filepath)
		},
	}
}
